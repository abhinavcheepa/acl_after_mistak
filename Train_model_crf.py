...
Fixes applied:
    ...
    [FIX-F] 4-layer fusion + MLP architecture (matches inference_pipeline_crf.py)
"""
...
BEST_PATH       = os.path.join(CHECKPOINT_DIR, "best_crf.pt")
RESUME_PATH     = os.path.join(CHECKPOINT_DIR, "resume_crf.pt")

MODEL_NAME      = "google/muril-base-cased"

# Training hyperparams
BATCH_SIZE      = 8          # [FIX-B] reduced for VRAM
...
WEIGHT_DECAY    = 1e-2
WARMUP_RATIO    = 0.1
FREEZE_LAYERS   = 10       # freeze bottom N layers of MuRIL
EMA_DECAY       = 0.999
DROPOUT         = 0.1
SEED            = 42

# Architecture
NUM_LAYERS_FUSE = 4          # last 4 hidden states fuse
MLP_HIDDEN      = 768
NUM_TAGS        = 17

# Tag vocabulary (must match data)
TAG2IDX = {
...
class MuRIL_MLP_CRF(nn.Module):
    """
    MuRIL → 4-layer hidden state fusion → MLP(768→768→17) → CRF
    Matches inference_pipeline_crf.py architecture exactly.
    """
    def __init__(self, model_name=MODEL_NAME, num_tags=NUM_TAGS,
        ...
        # Freeze embedding + bottom N layers
        self._freeze_layers(freeze_layers)

        # Gradient checkpointing — [FIX-B] VRAM ~40% less
        # self.bert.gradient_checkpointing_enable()

        # Learnable fusion weights over last num_layers_fuse hidden states
    ...
    def forward(self, input_ids, attention_mask, token_type_ids=None, labels=None):
        ...
        # Fuse last num_layers_fuse hidden states
        hidden_states = outputs.hidden_states  # tuple of (num_layers+1, B, T, 768)
        ...
        # CRF mask — ignore padding
        mask = attention_mask.bool()

        if labels is not None:
            # Replace -100 with 0 for CRF (CRF needs valid tag indices)
...
def save_checkpoint(path, model, optimizer, scheduler, epoch, best_f1,
    ...
    """
    [FIX-A] Memory-efficient checkpoint.
    lightweight=True → sirf trainable params save (resume ke liye)
    lightweight=False → trainable params + optimizer + EMA (best ke liye)
    """
    gc.collect()
    torch.cuda.empty_cache()

    # Sirf trainable params save — frozen 208M params skip
    trainable_state = {
...
def load_checkpoint(path, model, optimizer=None, scheduler=None, ema=None):
    """[FIX-D] strict=False — frozen params already in model, only trainable loaded."""
    ckpt = torch.load(path, map_location="cpu")

    # Load only the saved (trainable) params — frozen already initialized
    ...
    if ema and "ema_shadow" in ckpt:
        ...
        # Reinit EMA from current model if shadow missing
        ema.shadow = {
            k: v.clone().detach().cpu().float()
            for k, v in model.state_dict().items()
            if v.requires_grad
...
def compute_f1(preds_flat, labels_flat):
    """Macro F1 over all tags (ignore -100)."""
    from collections import defaultdict
    tp = defaultdict(int)
    fp = defaultdict(int)
    fn = defaultdict(int)
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # preds is list of lists (CRF decode)
            for i, pred_seq in enumerate(preds):
                label_seq = labels[i].tolist()
                for t, (p, l) in enumerate(zip(pred_seq, label_seq)):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume", action="store_true",
                        help="Resume training from resume_crf.pt")
